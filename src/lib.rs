...
pub async fn run() {
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        web_sys::window()
            ...
            .and_then(|doc| {
                let dst = doc.get_element_by_id("wasm-connection")?;
...
impl State {
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        ...
        if let WindowEvent::KeyboardInput {
            ...
            self.shape_state = !self.shape_state;
            return true;
        }
        return false;
    }
    ...
    fn update(&mut self) {
        if self.shape_state{
            ...
            self.num_vertices = VERTICES_PENTAGON.len() as u32;
...
const VERTICES_PENTAGON: &[Vertex] = &[
    Vertex { position: [-0.0868241, 0.49240386, 0.0],   color: [1.0, 0.0, 0.0], uv: [0.4131759, 0.00759614], }, // A
    Vertex { position: [-0.49513406, 0.06958647, 0.0],  color: [0.0, 1.0, 0.0], uv: [0.0048659444, 0.43041354], }, // B
    Vertex { position: [-0.21918549, -0.44939706, 0.0], color: [0.0, 0.0, 1.0], uv: [0.28081453, 0.949397], }, // C
    Vertex { position: [0.35966998, -0.3473291, 0.0],   color: [0.0, 0.0, 0.0], uv: [0.85967, 0.84732914], }, // D
    Vertex { position: [0.44147372, 0.2347359, 0.0],    color: [1.0, 1.0, 1.0], uv: [0.9414737, 0.2652641], }, // E
];
    ...
    fn desc<'a>() -> wgpu::VertexBufferLayout<'a> {
        wgpu::VertexBufferLayout {
            ...
            attributes: &[
                ...
                wgpu::VertexAttribute {
                    offset: std::mem::size_of::<[f32; 3]>() as wgpu::BufferAddress + std::mem::size_of::<[f32; 2]>() as wgpu::BufferAddress,
                    shader_location: 2,
                    format: wgpu::VertexFormat::Float32x3,
                },
            ]
